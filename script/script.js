...
function reset() {
	//reloads the windows to reset the pin cracker
...
document.getElementById("submitHash").onclick = function codecracker() {
	...
	if (result == md5) {
		// if the user pasted in the correct hash, continue, else display a warning
		start = Date.now();
		codecracker1(numbers, md5);
		if (bool != true) {
			//if pin isn't all numbers, check letters
		...
		if (bool == true) {
			// when hash is decoded, move to final step
		...
		// if user doesn't paste in the correct hash, display a warning
...
*/
function codecracker1(input, md5) {
	//this function takes the input and cracks the code, then returns it to the other function
